...
%
%  X is the input greyscale image, X should have zero mean
...
n = 3 %default to use 3 layers of dwt
Yq = nlevdwt(X,n)

% Quantise to integers. Rise1 set to step for default setting. For dwt, quantise each of the sub-images
% individually
fprintf(1, 'Quantising to step size of %i\n', qstep); 
% Yq=quant1(Yq,qstep,qstep);

%for the frst low pass image
size_A = 2^(8-n)
t=1:size_A 
Yq(t,t) = quant1(Yq(t,t),qstep(1,n+1),qstep(1,n+1))

%for the other high pass images 
for i = n:-1:1
    size_A = 2^(8-i)
    t=1:size_A 
    Yq(size_A+t,t) = quant1(Yq(t+size_A,t),qstep(1,i),qstep(1,i))
    Yq(t,t+size_A) = quant1(Yq(t,t+size_A),qstep(2,i),qstep(2,i))
    Yq(size_A+t,t+size_A) = quant1(Yq(t+size_A,t+size_A),qstep(3,i),qstep(3,i)) 
end

Yq = dwtgroup(Yq,n) %group the transformed image to DCT format
