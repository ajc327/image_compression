function Z = lbtdec2lev(vlc,vlc2, qstep,rise1, N, M, qf, bits, huffval, dcbits, W, H)

% lbtdec Decodes a (simplified) JPEG bit stream to an image using the
% quantization  table
%
...
%  regrouped.
%  reptable is the repeated quantisation  table
...
my_table= gen_quant_table2(qstep,qf); 

%this repeats the quantisation table so that it becomes 256 by 256 and can
%be applied directly to the output of the lbt 
reptable = repmat(my_table,size(Zq,1)/8);
Zi=quant2_table(Zq,qstep,qstep*rise1,reptable);
